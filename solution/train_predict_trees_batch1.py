...
print("(*) svd transformation for bert 768-d")
...
print("(*) join features")
X = hstack((rboc12, rboc21, csr_matrix(train_stat_features), csr_matrix(train_ex_features), csr_matrix(ge), csr_matrix(dd), csr_matrix(Xmin), x_svd)).tocsr()
X_test = hstack((rboc12_test, rboc21_test, csr_matrix(test_stat_features), csr_matrix(test_ex_features), csr_matrix(ge_test), csr_matrix(dd_test), csr_matrix(Xmin_test), xtest_svd)).tocsr()
print("dimension check", X.shape, X_test.shape)

print("(*) cleanup unused variables")
del rboc12, rboc21, train_stat_features, train_ex_features, ge, dd, Xmin, x_svd
del rboc12_test, rboc21_test, test_stat_features, test_ex_features, ge_test, dd_test, Xmin_test, xtest_svd
gc.collect()
...
### TRAIN - PREDICT ###
st = time.time()
print("(*) train cpu_test_results_lgb_nl32_mcs32_md8_lr002_ne600_ff08_bf08_rs24_knn31_extra33_boc_svd168")
model = lgb.LGBMClassifier(objective='multiclass', metric='softmax', boosting='gbdt', \
                           class_weight={0: 1., 1: 3., 2: 0.9},\
                           min_child_samples=32,\
                           num_leaves=32, max_depth=8, learning_rate=0.02, n_estimators=600, feature_fraction=0.8, bagging_fraction=0.8, nthread=-1, random_state=24)
        
preds, preds_test = validate_predict(model, X,y, X_test, n_splits=5, model_type='lgb', seed=24)
print(f"Time {time.time() - st:.02f}s")

with open("../sub/cpu_test_results_lgb_nl32_mcs32_md8_lr002_ne600_ff08_bf08_rs24_knn31_extra33_boc_svd168.tsv", 'w') as fo:
    for p in preds_test:
...
preds, preds_test = validate_predict(model, X,y, X_test, n_splits=10, model_type='lgb', seed=24)
print(f"Time {time.time() - st:.02f}s")

with open("../sub/cpu_test_results_lgb_nl32_mcs32_md8_lr002_ne600_ff08_bf08_rs24_knn31_extra33_boc_svd168_cv10.tsv", 'w') as fo:
    for p in preds_test:
        fo.write(f"{p[0]}\t{p[1]}\t{p[2]}\n")

### CHANGE TO CATBOOST ###
st = time.time()
print("(*) convert to dense array for catboost ")
X, X_test = X.toarray(), X_test.toarray()
X, X_test = np.nan_to_num(X), np.nan_to_num(X_test)
y2 = np.array([0 if label == 'agreed' else 1 if label == 'disagreed' else 2 for label in y])
print(X.shape, y.shape, X_test.shape)

print("(*) train cpu_test_results_cb_d6_it1000_lr002_l2lr3_rsm1_bc128_mcc2_rs24_knn31_extra33_boc_svd168_weighted_cv5_bestmodel")
model = cb.CatBoostClassifier(loss_function='MultiClass', depth=6, iterations=1000, learning_rate=0.02, l2_leaf_reg=3., rsm=1, \
                              border_count=128, max_ctr_complexity=2, random_seed=24, eval_metric='MultiClass', \
                              od_type='Iter', od_wait=5, nan_mode='Min', thread_count=16, logging_level='Silent', class_weights=[0.21,0.6,0.19], use_best_model=True)

preds, preds_test = validate_predict(model, X,y2, X_test, model_type='cb', seed=24, n_splits=5)
print(f"Time {time.time() - st:.02f}s")

with open("../sub/cpu_test_results_cb_d6_it1000_lr002_l2lr3_rsm1_bc128_mcc2_rs24_knn31_extra33_boc_svd168_weighted_cv5_bestmodel.tsv", 'w') as fo:
    for p in preds_test:
        fo.write(f"{p[0]}\t{p[1]}\t{p[2]}\n")
        
st = time.time()
print("(*) train cpu_test_results_cb_d6_it1000_lr002_l2lr3_rsm1_bc128_mcc2_rs24_knn31_extra33_boc_svd168_weighted_cv10_bestmodel")
model = cb.CatBoostClassifier(loss_function='MultiClass', depth=6, iterations=1000, learning_rate=0.02, l2_leaf_reg=3., rsm=1, \
                              border_count=128, max_ctr_complexity=2, random_seed=24, eval_metric='MultiClass', \
                              od_type='Iter', od_wait=5, nan_mode='Min', thread_count=16, logging_level='Silent', class_weights=[0.21,0.6,0.19], use_best_model=True)

preds, preds_test = validate_predict(model, X,y2, X_test, model_type='cb', seed=24, n_splits=10)
print(f"Time {time.time() - st:.02f}s")

with open("../sub/cpu_test_results_cb_d6_it1000_lr002_l2lr3_rsm1_bc128_mcc2_rs24_knn31_extra33_boc_svd168_weighted_cv10_bestmodel.tsv", 'w') as fo:
    for p in preds_test:
        fo.write(f"{p[0]}\t{p[1]}\t{p[2]}\n")
...
st = time.time()
print("(*) train")
model = LogisticRegression(C=1, random_state=42, solver='sag', n_jobs=4, class_weight='balanced')
        
preds, preds_test = validate_predict(model, X,y, X_test, n_splits=5, model_type='lor', seed=24)
print(f"Time {time.time() - st:.02f}s")

with open("../sub/cpu_test_results_lr_C1_sag_rs42_knn31_extra33_boc_svd168_cv5.tsv", 'w') as fo:
    for p in preds_test:
        fo.write(f"{p[0]}\t{p[1]}\t{p[2]}\n")
